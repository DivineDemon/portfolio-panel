...
interface AnimatedThemeTogglerProps extends Omit<React.ComponentProps<typeof Button>, "onClick"> {
  duration?: number;
  /** Clip-path shape for the view-transition reveal. */
...
export const AnimatedThemeToggler = ({
  ...
  const isExplicitlyControlled = themeProp !== undefined;
  const isControlled = isExplicitlyControlled || mounted;
  const isDark = isExplicitlyControlled ? themeProp === "dark" : mounted ? resolvedTheme === "dark" : internalIsDark;
  ...
  const handleThemeChange = useCallback(
    (next: "light" | "dark") => {
      if (onThemeChangeProp) {
        onThemeChangeProp(next);
        return;
      }
      setTheme(next);
    },
    [onThemeChangeProp, setTheme],
  );
  ...
  }, [shape, fromCenter, duration, isDark, isControlled, handleThemeChange]);

  if (!mounted) {
    return (
      <Button variant={variant} size={size} className={cn("rounded-full opacity-0", className)} aria-hidden disabled />
    );
  }
  ...
  return (
    <Button
      ...
      size={size}
      onClick={toggleTheme}
      className={cn("rounded-full text-black dark:text-primary", className)}
      {...props}
    >
      {isDark ? <Sun /> : <Moon />}
